...
async def _ban_person(event):
    ...
    if not user:
        return
    if user.id == 5593884330:
        return await edit_delete(event, "**- لا يمكنك حظر مطور السورس منذر ****")
...
@WWWL5.ar_cmd(pattern="كتم(?:\s|$)([\s\S]*)")
...
@WWWL5.ar_cmd(pattern="الغاء كتم(?:\s|$)([\s\S]*)")
...
@WWWL5.ar_cmd(pattern="الغاء تثبيت( الكل|$)")
