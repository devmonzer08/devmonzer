...
@WWWL5.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
        "⩹⌯⊷━♢ ⦓ ƚᥱᥣᥱƚ𝗁᥆ꪀ LEᗩᗪEᖇ ⦔ ♢━⊶⌯⩺\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب القائد حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب القائد الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين\n⩹⌯⊷━♢ ⦓ ƚᥱᥣᥱƚ𝗁᥆ꪀ LEᗩᗪEᖇ ⦔ ♢━⊶⌯⩺\n**⌯︙ [┈┉━｢ ᥉᥆υᖇᥴᥱ LEᗩᗪEᖇ 🕷 ｣━┅┈](t.me/EE_20)**",
        link_preview=False,
    )
...
@WWWL5.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@WWWL5.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@WWWL5.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@WWWL5.ar_cmd(pattern="المتحكمين$")
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم لالقائد الخاص بك**"
        )
    result = "**قائمة المستخدمين المتحكمين بتنصيب القائد :**\n\n"
