...
@WWWL5.ar_cmd(pattern="الصيد")
async def _(event):
    await event.edit(
        """
⩹⌯⊷━♢ ⦓ ƚᥱᥣᥱƚ𝗁᥆ꪀ LEᗩᗪEᖇ ⦔ ♢━⊶⌯⩺
أوامر الصيد الخاصة بسورس القائد : 
...
⩹⌯⊷━♢ ⦓ ƚᥱᥣᥱƚ𝗁᥆ꪀ LEᗩᗪEᖇ ⦔ ♢━⊶⌯⩺
⌯︙ [┈┉━｢ ᥉᥆υᖇᥴᥱ LEᗩᗪEᖇ ✰, ｣━┅┈](t.me/V_P_N_8)

"""
    )


@WWWL5.ar_cmd(pattern="صيد (.*)")
async def hunterusername(event):
    choice = str(event.pattern_match.group(1))
    try:
        ch = await WWWL5(
            functions.channels.CreateChannelRequest(
                title="leader HUNTER - صيد القائد",
                about="This channel to hunt username by - @V_P_N_8 ",
            )
        )
...
async def _(event):
    ...
    isclaim.clear()
    isclaim.append("off")
    await WWWL5.send_message(event.chat_id, "**- تم الانتهاء من التثبيت بنجاح**")


@WWWL5.ar_cmd(pattern="حالة الصيد")
async def _(event):
    if "on" in isclaim:
...
@WWWL5.ar_cmd(pattern="حالة التثبيت")
async def _(event):
    if "on" in isauto:
