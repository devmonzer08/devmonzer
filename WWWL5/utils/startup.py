...
LOGS = logging.getLogger("اعداد القائد")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await WWWL5(UnblockRequest("@Source_Monzer"))
        await WWWL5(
            UpdateNotifySettingsRequest(
                peer="t.me/dev_monzer_BOT",
                settings=InputPeerNotifySettings(mute_until=2**31 - 1),
            )
        )
        await WWWL5.edit_folder("@dev_monzer_BOT", folder=1)  # عمل ارشيف للبوت
        await WWWL5(UnblockRequest("@dev_monzer_BOT"))
        await WWWL5(JoinChannelRequest("@V_P_N_8"))
        await WWWL5(JoinChannelRequest("@Help_Leader"))
        await WWWL5(JoinChannelRequest("@Source_Monzer"))
        await WWWL5(JoinChannelRequest("@leader_VIAS"))
        await WWWL5(JoinChannelRequest("@Help_Tele_Leader"))
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await WWWL5.send_message("@dev_monzer_BOT", "/start")
            await asyncio.sleep(1)
            await WWWL5.send_message(
                "@dev_monzer_BOT",
                "✰تم بنجاح تشغيل سورس القائد ✰",
            )
            await asyncio.sleep(1)
        ...
        try:
            if BOTLOG:
                await WWWL5.tgbot.send_file(
                    BOTLOG_CHATID,
                    "https://graph.org/file/8cff98e65feaee203cd6c.jpg",
                    caption="**شكرا لتنصيبك تيلثون القائد**\n**`لعرض الاوامر ارسل `.الاوامر**\n**اضغط على الزر ي الاسفل**",
        ...
        try:
            if BOTLOG:
                await WWWL5.tgbot.send_message(
                    BOTLOG_CHATID,
                    "**تم تشغيل سورس القائد بنجاح**\n➖➖➖➖➖➖➖➖➖➖\n**قناة السورس: @V_P_N_8**\n**قناة الكلايش: @Source_Monzer**\n**مطور السورس: @WWWL5**\n➖➖➖➖➖➖➖➖➖➖\n**لعرض الاوامر ارسل `.الاوامر`**",
                    buttons=[
                        (Button.url("كروب المساعدة", "https://t.me/Help_Leader"),)
                    ],
                )
        except Exception as e:
            LOGS.error(e)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "جروب اشعارات القائد", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "جروب تخزين القائد", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
