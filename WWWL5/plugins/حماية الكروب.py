...
@WWWL5.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@WWWL5.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
    input_str = event.pattern_match.group(1)
    chat_id = event.chat_id
    if not event.is_group:
...
@WWWL5.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
...
@WWWL5.ar_cmd(incoming=True, forword=None)
async def check_incoming_messages(event):
    ...
    if is_locked(chat_id, "rtl") and (
        ...
        if WWWL5 == razan or await is_admin(event, WWWL5) or WWWL5 in devs:
            return
        else:
            try:
                await event.delete()
...
@WWWL5.ar_cmd(pattern=f"البوتات ?(.*)")
